...
use crate::traits::{Vfs, VfsFile, VfsMetadata, VfsPermissions, OpenOptionsConfig};
...
impl VfsPermissions for NativePermissions {
    fn readonly(&self) -> bool { self.0.readonly() }
    fn set_readonly(&mut self, readonly: bool) { self.0.set_readonly(readonly); }
    fn mode(&self) -> u32 {
        #[cfg(unix)]
        {
...
impl VfsMetadata for NativeMetadata {
    ...
    fn is_symlink(&self) -> bool { self.0.file_type().is_symlink() }
...
impl Vfs for NativeVfs {
    ...
    fn write(&self, path: &Path, contents: &[u8]) -> io::Result<()> { fs::write(path, contents) }
    ...
    fn create_dir(&self, path: &Path) -> io::Result<()> { fs::create_dir(path) }
    fn create_dir_all(&self, path: &Path) -> io::Result<()> { fs::create_dir_all(path) }
    fn read_link(&self, path: &Path) -> io::Result<PathBuf> { fs::read_link(path) }
...
impl VfsFile for NativeWrappedFile {
    fn set_len(&self, size: u64) -> io::Result<()> { self.0.set_len(size) }
